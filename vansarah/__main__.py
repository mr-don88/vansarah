"""vansarah TTS CLI
Example usage:
python3 -m vansarah --text "The sky above the port was the color of television, tuned to a dead channel." -o file.wav --debug

echo "Bom dia mundo, como vão vocês" > text.txt
python3 -m vansarah -i text.txt -l p --voice pm_alex > audio.wav
...
def generate_audio(
    text: str, vansarah_language: str, voice: str, speed=1
) -> Generator["KPipeline.Result", None, None]:
    from vansarah import KPipeline

    if not voice.startswith(vansarah_language):
        logger.warning(f"Voice {voice} is not made for language {vansarah_language}")
    pipeline = KPipeline(lang_code=vansarah_language)
    yield from pipeline(text, voice=voice, speed=speed, split_pattern=r"\n+")


def generate_and_save_audio(
    output_file: Path, text: str, vansarah_language: str, voice: str, speed=1
) -> None:
    with wave.open(str(output_file.resolve()), "wb") as wav_file:
        ...
        for result in generate_audio(
            text, vansarah_language=vansarah_language, voice=voice, speed=speed
        ):
